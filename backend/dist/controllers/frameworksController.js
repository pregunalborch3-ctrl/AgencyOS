...
exports.mapCompetition = mapCompetition;
exports.planDistribution = planDistribution;
exports.viralContent = viralContent;
exports.scalingRoadmap = scalingRoadmap;
...
async function analyzeMarket(req, res) {
    try {
        ...
        const data = await claudeJSON('Eres un analista de mercado experto en e-commerce y marketing digital. Responde ÚNICAMENTE con JSON válido, sin markdown, sin comentarios.', `Analiza este nicho de mercado: "${nicho}"
...
async function mapCompetition(req, res) {
    try {
        ...
        const data = await claudeJSON('Eres un analista de competencia experto en estrategia de negocio y marketing. Responde ÚNICAMENTE con JSON válido, sin markdown.', `Analiza estos competidores en la industria "${industry}": ${competitors.filter(Boolean).join(', ')}
...
async function planDistribution(req, res) {
    try {
        ...
        const data = await claudeJSON('Eres un experto en marketing digital y distribución multicanal. Responde ÚNICAMENTE con JSON válido, sin markdown.', `Crea un plan de distribución de 30 días para:
- Producto: ${product}
...
async function viralContent(req, res) {
    try {
        const { niche, tone, platforms } = req.body;
        ...
        const data = await claudeJSON('Eres un experto en contenido viral y growth hacking para redes sociales. Responde ÚNICAMENTE con JSON válido, sin markdown.', `Crea una estrategia de contenido viral para:
- Nicho: ${niche}
- Tono: ${tone}
...
async function scalingRoadmap(req, res) {
    try {
        const { currentRevenue, target, timeframe } = req.body;
        ...
        const data = await claudeJSON('Eres un experto en escalado de negocios digitales y agencias de marketing. Responde ÚNICAMENTE con JSON válido, sin markdown.', `Crea un roadmap de escalado para:
